...
class ChatController extends GetxController {
  ...
  void onInit() {
    ...
    // ✅ ما عاد في حاجة لـ stream منفصل: WebSocketService بينادي
    // handleIncomingMessage() مباشرة لما يوصل حدث 'send_message'
    // (شوف websocket_service.dart -> _handleNewMessage).
  }
  ...
  /// تُستدعى من WebSocketService عند وصول حدث 'send_message'.
  /// هي المسؤولة عن تحديد إذا الرسالة الواردة تخص هاي المحادثة بالتحديد
  /// (بمقارنة sender مع otherPartyProfile)، وإذا نعم بتعمل إعادة تحميل
  /// للرسائل. اعتمدنا refetch كامل بدل الإدراج اليدوي لتفادي الاعتماد
  /// على شكل payload غير مؤكد قادم من الباك.
  void handleIncomingMessage(Map data) {
    final senderId = data['content'] is Map ? data['content']['sender'] : null;

    final isForThisConversation = senderId != null &&
        (senderId == otherPartyProfile.userId ||
  ...
  Future<void> pickAndSendImage() async {
    ...
    if (pickedFile != null) {
      ...
      final response =
          await _chatRemote.sendFileMessage(conversationId, file);
      statusRequest = handlingData(response);
      if (statusRequest == StatusRequest.success) {
        final newMessage = MessageModel.fromJson(response['data']);
        messages.insert(0, newMessage);

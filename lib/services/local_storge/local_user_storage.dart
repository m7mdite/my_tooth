...
// ==============================================================
  ...
  Future<void> saveId(String id) async =>
      await _box.write('${_profilePrefix}id', id);
  String? getId() => _box.read('${_profilePrefix}id');

  Future<void> saveUserId(String userId) async =>
      await _box.write('${_profilePrefix}user_id', userId);
  String? getUserId() => _box.read('${_profilePrefix}user_id');

  Future<void> saveFirstName(String name) async =>
      await _box.write('${_profilePrefix}first_name', name);
  String? getFirstName() => _box.read('${_profilePrefix}first_name');

  Future<void> saveFatherName(String name) async =>
      await _box.write('${_profilePrefix}father_name', name);
  String? getFatherName() => _box.read('${_profilePrefix}father_name');

  Future<void> saveLastName(String name) async =>
      await _box.write('${_profilePrefix}last_name', name);
  String? getLastName() => _box.read('${_profilePrefix}last_name');

  Future<void> saveEmail(String email) async =>
      await _box.write('${_profilePrefix}email', email);
  String? getEmail() => _box.read('${_profilePrefix}email');

  Future<void> saveBio(String bio) async =>
      await _box.write('${_profilePrefix}bio', bio);
  String? getBio() => _box.read('${_profilePrefix}bio');

  Future<void> savePhoneNumber(String phone) async =>
      await _box.write('${_profilePrefix}phone_number', phone);
  String? getPhoneNumber() => _box.read('${_profilePrefix}phone_number');

  Future<void> saveAge(String age) async =>
      await _box.write('${_profilePrefix}age', age);
  String? getAge() => _box.read('${_profilePrefix}age');

  Future<void> saveGender(String gender) async =>
      await _box.write('${_profilePrefix}gender', gender);
  String? getGender() => _box.read('${_profilePrefix}gender');

  Future<void> saveUniversityNumber(String num) async =>
      await _box.write('${_profilePrefix}university_number', num);
  String? getUniversityNumber() =>
      _box.read('${_profilePrefix}university_number');

  Future<void> saveCategory(String cat) async =>
      await _box.write('${_profilePrefix}category', cat);
  String? getCategory() => _box.read('${_profilePrefix}category');

  Future<void> saveIsVerified(bool verified) async =>
      await _box.write('${_profilePrefix}is_verified', verified);
  bool isVerified() => _box.read('${_profilePrefix}is_verified') ?? false;

  Future<void> saveCompletedCases(int count) async =>
      await _box.write('${_profilePrefix}completed_cases', count);
  int getCompletedCases() => _box.read('${_profilePrefix}completed_cases') ?? 0;

  Future<void> saveInProgressCases(int count) async =>
      await _box.write('${_profilePrefix}in_progress_cases', count);
  int getInProgressCases() =>
      _box.read('${_profilePrefix}in_progress_cases') ?? 0;

  Future<void> saveProfilePhotoPublicId(String? id) async =>
      await _box.write('${_profilePrefix}photo_public_id', id);
  String? getProfilePhotoPublicId() =>
      _box.read('${_profilePrefix}photo_public_id');

  Future<void> saveProfilePhotoUrl(String? url) async =>
      await _box.write('${_profilePrefix}photo_url', url);
  String? getProfilePhotoUrl() => _box.read('${_profilePrefix}photo_url');

  Future<void> saveProfilePicture(String url) async =>
      await _box.write('${_profilePrefix}profile_picture', url);
  ...
  Future<void> clearProfileData() async {
    final keys =
        _box.getKeys().where((k) => k.toString().startsWith(_profilePrefix));
    for (var key in keys) {

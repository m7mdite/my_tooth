...
// import 'package:gr_flutter/controllers/conversations_controllers/chat_controller.dart';
...
//     print('⚠️ Notifications not supported on Windows, skipping.');
//     return;
//   }
...
//     // يمكنك أيضاً تخزين الإشعار في قاعدة محلية أو تحديث العداد
//   });
// }

//   void _registerEventHandlers() {
//     _socket.on('connect', _handleConnect);
//     _socket.on('notify', _listenToEvents);
...
class WebSocketService extends GetxService {
  ...
  // ✅ تم تصحيح التوقيع: كانت الدالة بدون باراميتر وهذا يسبب خطأ
  // "The argument type 'void Function()' can't be assigned to the
  // parameter type 'EventHandler<dynamic>'"
  // لازم تستقبل باراميتر واحد (dynamic data) عشان تنسجم مع EventHandler.
  //
  // ✅ ملاحظة مهمة: الباك عندك يرسل الحدث باسم 'noify' (فيه خطأ إملائي):
  //   io.to(user.id.toString()).emit('noify', { message: content });
  // الأفضل تصحيح الاسم في الباك إلى 'notify' حتى يتطابق مع الفرونت.
  void _handleNotify(dynamic data) {
    debugPrint('📩 notify: $data');

    // عرض الإشعار للمستخدم
    Get.snackbar(
      'إشعار جديد',
      data is Map ? (data['message'] ?? 'لديك إشعار جديد') : 'لديك إشعار جديد',
      snackPosition: SnackPosition.TOP,
      duration: const Duration(seconds: 5),
    );

    // يمكنك أيضاً تخزين الإشعار في قاعدة محلية أو تحديث العداد عبر
    // _showSystemNotification / _saveToLocalStorage إذا احتجت لاحقاً.
  }

  void _registerEventHandlers() {
    _socket.on('connect', _handleConnect);
    _socket.on('notify', _handleNotify);
    _socket.on('requestAccepted', _handleRequestAccepted);
    _socket.on('VerifyAccepted', _handleVerifyAccepted);
    _socket.on('studentSelectedOverseer', _handleStudentSelectedOverseer);
    _socket.on('disconnect', _handleDisconnect);

...
class AuthService {
  ...
  // بادئة للمفاتيح الخاصة بالبروفايل
  ...
  // --- الأساسيات ---
  Future<void> saveId(String id) async => await _box.write('${_prefix}id', id);
  String? getId() => _box.read('${_prefix}id');
  
  Future<void> saveUserId(String userId) async => await _box.write('${_prefix}user_id', userId);
  String? getUserId() => _box.read('${_prefix}user_id');
  
  Future<void> saveFirstName(String firstName) async => await _box.write('${_prefix}first_name', firstName);
  String? getFirstName() => _box.read('${_prefix}first_name');
  
  Future<void> saveFatherName(String fatherName) async => await _box.write('${_prefix}father_name', fatherName);
  String? getFatherName() => _box.read('${_prefix}father_name');
  
  Future<void> saveLastName(String lastName) async => await _box.write('${_prefix}last_name', lastName);
  String? getLastName() => _box.read('${_prefix}last_name');
  
  Future<void> saveEmail(String email) async => await _box.write('${_prefix}email', email);
  String? getEmail() => _box.read('${_prefix}email');
  
  Future<void> saveBio(String bio) async => await _box.write('${_prefix}bio', bio);
  String? getBio() => _box.read('${_prefix}bio');
  
  Future<void> savePhoneNumber(String phoneNumber) async => await _box.write('${_prefix}phone_number', phoneNumber);
  String? getPhoneNumber() => _box.read('${_prefix}phone_number');
  
  Future<void> saveAge(String age) async => await _box.write('${_prefix}age', age);
  String? getAge() => _box.read('${_prefix}age');
  
  Future<void> saveGender(String gender) async => await _box.write('${_prefix}gender', gender);
  String? getGender() => _box.read('${_prefix}gender');
  
  Future<void> saveUniversityNumber(String universityNumber) async => await _box.write('${_prefix}university_number', universityNumber);
  String? getUniversityNumber() => _box.read('${_prefix}university_number');
  
  // --- الفئة (للطالب) ---
  Future<void> saveCategory(String category) async => await _box.write('${_prefix}category', category);
  String? getCategory() => _box.read('${_prefix}category');
  
  // --- حالة التوثيق ---
  Future<void> saveIsVerified(bool isVerified) async => await _box.write('${_prefix}is_verified', isVerified);
  bool isVerified() => _box.read('${_prefix}is_verified') ?? false;
  
  // --- إحصائيات الحالات (للمشرف والطالب) ---
  Future<void> saveCompletedCases(int count) async => await _box.write('${_prefix}completed_cases', count);
  int getCompletedCases() => _box.read('${_prefix}completed_cases') ?? 0;
  
  Future<void> saveInProgressCases(int count) async => await _box.write('${_prefix}in_progress_cases', count);
  int getInProgressCases() => _box.read('${_prefix}in_progress_cases') ?? 0;
  
  // --- صورة البروفايل ---
  Future<void> saveProfilePhotoPublicId(String? publicId) async => await _box.write('${_prefix}photo_public_id', publicId);
  String? getProfilePhotoPublicId() => _box.read('${_prefix}photo_public_id');
  
  Future<void> saveProfilePhotoUrl(String? url) async => await _box.write('${_prefix}photo_url', url);
  String? getProfilePhotoUrl() => _box.read('${_prefix}photo_url');
  
  Future<void> saveProfilePicture(String url) async => await _box.write('${_prefix}profile_picture', url);
  String? getProfilePicture() => _box.read('${_prefix}profile_picture');
  
  // --- التوكن والدور ---
  Future<void> saveToken(String token) async => await _box.write(_tokenKey, token);
  String? getToken() => _box.read(_tokenKey);
  
  Future<void> saveRole(String role) async => await _box.write(_roleKey, role);
  String? getRole() => _box.read(_roleKey);
  
  // --- إصدار البيانات (اختياري) ---
  Future<void> saveV(int v) async => await _box.write('${_prefix}v', v);
  int? getV() => _box.read('${_prefix}v');
  ...
  String getFullName() {
    ...
    return [firstName, fatherName, lastName].where((n) => n.isNotEmpty).join(' ');
  }
  
  // التحقق من وجود توكن
  bool hasToken() => getToken() != null && getToken()!.isNotEmpty;
  ...
  // مسح بيانات البروفايل فقط (دون مسح التوكن والدور)
  Future<void> clearProfileData() async {
    final keys = [
      '${_prefix}id', '${_prefix}user_id', '${_prefix}first_name', '${_prefix}father_name',
      '${_prefix}last_name', '${_prefix}email', '${_prefix}bio', '${_prefix}phone_number',
      '${_prefix}age', '${_prefix}gender', '${_prefix}university_number', '${_prefix}category',
      '${_prefix}is_verified', '${_prefix}completed_cases', '${_prefix}in_progress_cases',
      '${_prefix}photo_public_id', '${_prefix}photo_url', '${_prefix}profile_picture', '${_prefix}v'
    ];
    for (var key in keys) await _box.remove(key);
  }
  
  // مسح بيانات المستخدم بالكامل (بما فيها التوكن والدور)
  ...
  Future<void> saveAllFromJson(Map<String, dynamic> json) async {
    try {
      ...
      // الفئة (قد تأتي كـ String أو Map)
      ...
      print('✅ All profile data saved to local storage');
    } catch (e) {
  ...
  // الحصول على جميع البيانات كـ Map (للاستخدام في حالة الحاجة)
  Map<String, dynamic> getAllAsJson() {
    return {
      ...
      'completed_cases': getCompletedCases(),
      'in_progress_cases': getInProgressCases(),
      'profile_photo': {
